...
enum ShareInfoService {
    // MARK: 목표내용 저장하기
    static func addShareContent(nickName: String, profile: String, code: String, content: [String]) async throws {
        ...
        let otherRef = docRef.document(userRef.documentID).collection(CollectionName.ShareGoal.rawValue).document(userData.myCode)
